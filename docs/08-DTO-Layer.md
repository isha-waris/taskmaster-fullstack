...
## Big Picture (keep this mental model)

→ Entities → Database structure (EF Core cares)
→ DTOs → API contract (clients care)
→ Controller → Translates DTO ⇄ Entity
→ Repository → Works only with Entities
👉 Entities NEVER go directly to API responses
